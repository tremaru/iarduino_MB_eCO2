...
// На шине должен быть только один модуль углекислого газа.                                        //
...
//   Находим текущий адрес модуля углекислого газа, скорость и тип протокола Modbus:               //
...
//   - если скорость известна, то строки с другими скоростями можно удалить.                       //
             ...
             Serial.print("Поиск модуля углекислого газа...");                                     //
     ...
     if(!f){ rs485.begin( 57600); while(!rs485); delay(10); Serial.print('.'); f=sensor.begin(); } //   Инициируем работу с модулем на скорости  57600, без указания адреса (адрес будет найден автоматически).
     if(!f){ rs485.begin(115200); while(!rs485); delay(10); Serial.print('.'); f=sensor.begin(); } //   Инициируем работу с модулем на скорости 115200, без указания адреса (адрес будет найден автоматически).
//   Выводим результат обнаружения модуля углекислого газа:                                        //
     if(!f){ Serial.println("\r\nМодуль углекислого газа не найден"); return; }                    //
     else  { Serial.println("\r\nМодуль углекислого газа найден"   );         }                    //
//   Меняем адрес модуля углекислого газа:                                                         //
     f =     sensor.changeID(newAddress);                                                          //
     if(!f){ Serial.println("Адрес не изменён"); return; }                                         //
//   Устанавливаем новую скорость передачи данных модуля углекислого газа:                         //
...
//   Подтверждаем изменение скорости передачи данных модуля углекислого газа:                      //
...
//   Устанавливаем новый тип протокола модуля углекислого газа:                                    //
...
//   Подтверждаем изменение типа протокола модуля углекислого газа:                                //
...
//   Выводим сообшение об успешном изменении адреса, скорости и типа протокола:                    //
             ...
             Serial.println((String) "Адрес    = "+sensor.getID()                        );        //   Выводим адрес модуля углекислого газа.
             Serial.println((String) "Версия   = "+sensor.getVersion()                   );        //   Выводим версию прошивки модуля углекислого газа.
}                                                                                                  //
                                                                                                   //
void loop(){                                                                                       //
